...
import com.pennywiseai.tracker.ui.components.PendingCategoryReviewSheet
...
import com.pennywiseai.tracker.ui.viewmodel.PendingCategoryReviewViewModel
...
fun PennyWiseApp(
    ...
    val pendingReviewViewModel: PendingCategoryReviewViewModel = hiltViewModel(viewModelStoreOwner = owner, key = null)
    ...
    val pendingReviewIds by pendingReviewViewModel.pendingIds.collectAsStateWithLifecycle()
    ...
    PennyWiseTheme(
        ...
        // Real-time categorization prompt: when SMS transactions were just
        // saved into the fallback "Others" category, pop the shared category
        // picker over whatever screen is open — one transaction goes straight
        // to the picker, several open the batch review list first. Suppressed
        // while the app is locked so the sheet can't leak over the lock
        // screen; it resurfaces after unlock because the state survives. When
        // the app was backgrounded at ingest time the transaction
        // notification already carries quick-pick actions, and this still
        // fires on the next app open (stale events skipped).
        if (!appLockUiState.isLocked) {
            var pendingReviewEditingId by remember { mutableStateOf<Long?>(null) }
            val editing = pendingReviewEditingId
            when {
                editing != null -> QuickCategoryPickerHost(
                    transactionId = editing,
                    // Matches the txn-alert notification id (transactionId.toInt()),
                    // so picking here also clears that notification.
                    notificationId = editing.toInt(),
                    transactionRepository = pendingReviewViewModel.transactionRepository,
                    categoryRepository = pendingReviewViewModel.categoryRepository,
                    tagRepository = pendingReviewViewModel.tagRepository,
                    appScope = pendingReviewViewModel.appScope,
                    onFinished = {
                        pendingReviewViewModel.removeFromQueue(editing)
                        pendingReviewEditingId = null
                    }
                )
                pendingReviewIds.size == 1 -> QuickCategoryPickerHost(
                    transactionId = pendingReviewIds[0],
                    notificationId = pendingReviewIds[0].toInt(),
                    transactionRepository = pendingReviewViewModel.transactionRepository,
                    categoryRepository = pendingReviewViewModel.categoryRepository,
                    tagRepository = pendingReviewViewModel.tagRepository,
                    appScope = pendingReviewViewModel.appScope,
                    onFinished = { pendingReviewViewModel.removeFromQueue(pendingReviewIds[0]) }
                )
                pendingReviewIds.size > 1 -> PendingCategoryReviewSheet(
                    transactionIds = pendingReviewIds,
                    transactionRepository = pendingReviewViewModel.transactionRepository,
                    onPick = { pendingReviewEditingId = it },
                    onLater = { pendingReviewViewModel.clearQueue() }
                )
            }
        }
    }
