...
enum class TimePeriod(@StringRes val labelRes: Int) {
    THIS_MONTH(R.string.filter_time_this_month),
    LAST_MONTH(R.string.filter_time_last_month),
    CURRENT_FY(R.string.filter_time_current_fy),
    ALL(R.string.filter_time_all_time),
    CUSTOM(R.string.filter_time_custom_range);
 ...
 *
 * [label] is the localized text for the period (callers resolve
 * [TimePeriod.labelRes]); it is only used when the period has no resolved
