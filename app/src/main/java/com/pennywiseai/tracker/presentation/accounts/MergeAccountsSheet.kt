...
fun MergeAccountsSheet(
    ...
    ModalBottomSheet(onDismissRequest = onDismiss, sheetState = sheetState) {
        ...
        LazyColumn(
            ...
            item(key = "header") {
                Column {
                    Text(
                        text = stringResource(R.string.acct_merge_accounts),
                        style = MaterialTheme.typography.titleMedium,
                        fontWeight = FontWeight.SemiBold
                    )
                    Text(
                        text = stringResource(R.string.acct_merge_description),
            ...
            // Source picker
            item(key = "from-label") { SectionLabel(stringResource(R.string.acct_move_from)) }
            accountPickerItems(
                idPrefix = "src",
                accounts = accounts,
                selected = source,
            ...
            source?.let { src ->
                ...
                if (targets.isEmpty()) {
                    item(key = "no-targets") {
                        Text(
                            text = stringResource(R.string.acct_no_matching_accounts),
                            style = MaterialTheme.typography.bodySmall,
                    ...
                    item(key = "into-header") {
                        Column(modifier = Modifier.padding(top = Spacing.md)) {
                            SectionLabel(stringResource(R.string.acct_into))
                            Row(
    ...
    if (s != null && t != null) {
        AlertDialog(
            ...
            title = { Text(stringResource(R.string.acct_merge_accounts_question)) },
            text = {
                val n = sourceTxnCount
                Text(
                    if (n != null)
                        stringResource(R.string.acct_merge_confirm_count, n, AccountBalanceEntity.accountLabel(s.bankName, s.accountLast4), AccountBalanceEntity.accountLabel(t.bankName, t.accountLast4))
                    else
                        stringResource(R.string.acct_merge_confirm, AccountBalanceEntity.accountLabel(s.bankName, s.accountLast4), AccountBalanceEntity.accountLabel(t.bankName, t.accountLast4))
                )
            },
            confirmButton = {
                TextButton(onClick = { onConfirm(s, t) }) { Text(stringResource(R.string.acct_merge)) }
            },
            dismissButton = {
                TextButton(onClick = { target = null }) { Text(stringResource(R.string.acct_cancel)) }
            }
        )
    }
}
...
private fun AccountPickerRow(
    ...
    val creditLabel = stringResource(R.string.acct_credit)
    Surface(
        ...
        Row(
            ...
            Column(modifier = Modifier.weight(1f)) {
                ...
                Text(
                    text = buildString {
                        ...
                        if (acct.isCreditCard) append(" · " + creditLabel)
                    },
                    style = MaterialTheme.typography.bodySmall,
                    color = MaterialTheme.colorScheme.onSurfaceVariant
                )
            ...
            if (isSelected) {
                AssistChip(
                    onClick = {},
                    label = { Text(stringResource(R.string.acct_selected)) },
                    leadingIcon = {
                        Icon(
                            Icons.Default.Check,
                            contentDescription = null,
