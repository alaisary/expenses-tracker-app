...
class TransactionDetailViewModel @Inject constructor(
     ...
     * spending trends, averages, category/budget breakdowns and insights
     * ignore it.
     */
    ...
    fun addBalanceAdjustment() {
        ...
        viewModelScope.launch {
            try {
                ...
                _errorMessage.value = context.getString(R.string.vm_adjustment_failed, e.message)
            } finally {
    ...
    fun updateAmount(amountStr: String) {
        ...
        if (amount != null && amount > BigDecimal.ZERO) {
            ...
            _errorMessage.value = context.getString(R.string.vm_amount_must_be_positive_number)
        }
    }
        ...
        viewModelScope.launch {
            try {
                ...
                if (existing != null && existing.isIncome != isIncome) {
                    ...
                    val existingType = context.getString(
                        if (existing.isIncome) R.string.vm_type_income else R.string.vm_type_expense
                    )
                    _errorMessage.value =
                        context.getString(R.string.vm_category_exists_as_type, trimmed, existingType)
                    onResult(false)
                    return@launch
                }
                if (existing == null) {
                ...
                _errorMessage.value = context.getString(R.string.vm_category_create_failed, e.message)
                onResult(false)
            }
        }
    }
    ...
    fun enableSplitMode() {
        ...
        if (transaction.transactionType !in SPLITTABLE_TYPES) {
            _errorMessage.value = context.getString(R.string.vm_splits_expenses_only)
            return
        }
    ...
    fun validateSplits(): Boolean {
        ...
        if (currentSplits.size < 2) {
            _errorMessage.value = context.getString(R.string.vm_splits_min_two)
            return false
        }

        // All splits must have positive amounts
        if (currentSplits.any { it.amount <= BigDecimal.ZERO }) {
            _errorMessage.value = context.getString(R.string.vm_splits_amounts_positive)
            return false
        }
        ...
        if (difference > tolerance) {
            _errorMessage.value = context.getString(R.string.vm_splits_must_equal_total)
            return false
        }
    ...
    fun saveChanges() {
        ...
        if (toSave.merchantName.isBlank()) {
            _errorMessage.value = context.getString(R.string.vm_merchant_name_required)
            return
        }

        if (toSave.amount <= BigDecimal.ZERO) {
            _errorMessage.value = context.getString(R.string.vm_amount_must_be_positive)
            return
        }
        ...
        if (toSave.transactionType == TransactionType.TRANSFER &&
            ...
            _errorMessage.value = context.getString(R.string.vm_transfer_source_dest_differ)
            return
        }
            ...
            try {
                ...
                if (_applyTagsToAllFromMerchant.value) {
                    try {
                        ...
                        ruleError = context.getString(R.string.vm_tag_rule_create_failed, e.message)
                    }
                }
                ...
                _errorMessage.value = context.getString(R.string.vm_failed_save_changes, e.message)
            } finally {
        ...
        viewModelScope.launch {
            _transaction.value?.let { txn ->
                ...
                try {
                    ...
                    _errorMessage.value = context.getString(R.string.vm_failed_delete_transaction)
                } finally {
    ...
    fun createLoanFromTransaction(
        ...
        viewModelScope.launch {
            try {
                ...
                _errorMessage.value = context.getString(R.string.vm_failed_create_loan, e.message)
            }
        }
    }
    ...
    fun unlinkLoan() {
        ...
        viewModelScope.launch {
            try {
                ...
                _errorMessage.value = context.getString(R.string.vm_failed_unlink_loan, e.message)
            }
        }
    }
