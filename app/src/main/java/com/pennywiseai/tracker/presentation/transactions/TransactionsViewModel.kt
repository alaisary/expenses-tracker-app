...
class TransactionsViewModel @Inject constructor(
    ...
    fun bulkUpdateCategory(newCategory: String) {
        ...
        viewModelScope.launch {
            ...
            _bulkSnack.value = BulkSnack(
                message = context.getString(R.string.vm_bulk_updated_to, previous.size, newCategory),
                undo = {
                    viewModelScope.launch {
    ...
    fun bulkMarkAsTransfer() {
        ...
        if (ids.size != 2) {
            _bulkSnack.value = BulkSnack(context.getString(R.string.vm_transfer_select_exactly_two))
            return
        }
        ...
        if (!(hasExpense && hasIncome)) {
            _bulkSnack.value = BulkSnack(context.getString(R.string.vm_transfer_pick_one_each))
            return
        }
        clearSelection()
    ...
    fun markPairAsTransfer(aId: Long, bId: Long) {
        viewModelScope.launch {
            ...
            refreshWidgets()
            _bulkSnack.value = BulkSnack(
                message = context.getString(R.string.vm_marked_as_transfer),
                undo = {
                    viewModelScope.launch {
    ...
    fun bulkDelete() {
        ...
        viewModelScope.launch {
            ...
            clearSelection()
            _bulkSnack.value = BulkSnack(
                message = context.getString(R.string.vm_bulk_deleted, snapshot.size),
                undo = {
                    viewModelScope.launch {
    ...
    fun bulkAddToGroup(groupId: Long, groupName: String) {
        ...
        viewModelScope.launch {
            ...
            clearSelection()
            _bulkSnack.value = BulkSnack(
                message = context.getString(R.string.vm_bulk_added_to_group, previous.size, groupName),
    ...
    fun bulkCreateGroupAndAdd(name: String, note: String? = null) {
        ...
        viewModelScope.launch {
            ...
            clearSelection()
            _bulkSnack.value = BulkSnack(
                message = context.getString(R.string.vm_bulk_added_to_group, previous.size, name.trim()),
                undo = {
                    viewModelScope.launch {
...
enum class DateGroup(@StringRes val labelRes: Int) {
    ...
    YESTERDAY(R.string.filter_date_yesterday),
    THIS_WEEK(R.string.filter_date_this_week),
    EARLIER(R.string.filter_date_earlier)
}

enum class SortOption(@StringRes val labelRes: Int) {
    DATE_NEWEST(R.string.filter_sort_newest),
    DATE_OLDEST(R.string.filter_sort_oldest),
    AMOUNT_HIGHEST(R.string.filter_sort_amount_highest),
    AMOUNT_LOWEST(R.string.filter_sort_amount_lowest),
    MERCHANT_AZ(R.string.filter_sort_merchant_az),
    MERCHANT_ZA(R.string.filter_sort_merchant_za)
}
