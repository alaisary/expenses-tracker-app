...
class AddViewModel @Inject constructor(
    ...
    fun saveTransaction(onSuccess: () -> Unit) {
        ...
        if (isTransfer) {
            ...
            when {
                from == null || to == null ->
                    _transactionUiState.update { it.copy(error = appContext.getString(R.string.vm_transfer_select_both_accounts)) }
                from.id == to.id ->
                    _transactionUiState.update { it.copy(error = appContext.getString(R.string.vm_transfer_accounts_must_differ)) }
                from.currency != to.currency ->
                    _transactionUiState.update { it.copy(error = appContext.getString(R.string.vm_transfer_same_currency)) }
        ...
        viewModelScope.launch {
            try {
                ...
                _transactionUiState.update { currentState ->
                    currentState.copy(
                        isLoading = false,
                        error = e.message ?: appContext.getString(R.string.vm_failed_save_transaction)
                    )
                }
            }
        }
        ...
        viewModelScope.launch {
            try {
                ...
                _transactionUiState.update { currentState ->
                    currentState.copy(
                        isLoading = false,
                        error = e.message ?: appContext.getString(R.string.vm_failed_save_transfer)
                    )
                }
            }
        }
    ...
    fun updateSubscriptionService(service: String) {
        _subscriptionUiState.update { currentState ->
            currentState.copy(
                serviceName = service,
                serviceError = if (service.isBlank()) appContext.getString(R.string.vm_service_name_required) else null
            )
        }
    }
    ...
    fun saveSubscription(onSuccess: () -> Unit) {
        ...
        // Validate all fields
        val serviceError = if (state.serviceName.isBlank()) appContext.getString(R.string.vm_service_name_required) else null
        val amountError = validateAmount(state.amount)
        ...
        viewModelScope.launch {
            try {
                ...
                _subscriptionUiState.update { currentState ->
                    currentState.copy(
                        isLoading = false,
                        error = e.message ?: appContext.getString(R.string.vm_failed_save_subscription)
                    )
                }
            } finally {
    ...
    private fun validateAmount(amount: String): String? {
        return when {
            amount.isBlank() -> appContext.getString(R.string.vm_amount_required)
            amount.toDoubleOrNull() == null -> appContext.getString(R.string.vm_amount_invalid)
            amount.toDouble() <= 0 -> appContext.getString(R.string.vm_amount_must_be_greater_than_zero)
            else -> null
        }
    }
    ...
    private fun validateMerchant(merchant: String): String? {
        return when {
            merchant.isBlank() -> appContext.getString(R.string.vm_merchant_required)
            merchant.length < 2 -> appContext.getString(R.string.vm_too_short)
            else -> null
        }
    }
    ...
    private fun validateCategory(category: String): String? {
        return when {
            category.isBlank() -> appContext.getString(R.string.vm_category_required)
            else -> null
        }
    }
}
