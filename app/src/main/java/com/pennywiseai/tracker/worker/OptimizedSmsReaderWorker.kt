...
class OptimizedSmsReaderWorker @AssistedInject constructor(
    ...
    companion object {
        ...
        fun buildProgressNotification(context: Context, processed: Int, total: Int): android.app.Notification {
            ...
            if (nm.getNotificationChannel(channelId) == null) {
                nm.createNotificationChannel(
                    android.app.NotificationChannel(
                        channelId,
                        context.getString(R.string.wid_sms_scan_channel),
            ...
            return androidx.core.app.NotificationCompat.Builder(context, channelId)
                ...
                .setContentTitle(context.getString(R.string.wid_scanning_transactions))
                .setContentText(
                    if (total > 0) context.getString(R.string.wid_processed_progress, processed, total)
                    else context.getString(R.string.wid_reading_sms)
                )
    ...
    private suspend fun streamPipeline(
        ...
        // Retroactive internal-transfer pairing: repairs history already in the
        // DB (re-scans dedup-skip old rows, so per-insert pairing never sees
        // them). Idempotent — paired rows drop out of the candidate query.
        try {
            val paired = transactionRepository.pairHistoricInternalTransfers()
            if (paired > 0) {
                Log.i(TAG, "Paired $paired internal transfer pairs (both sides now TRANSFER)")
                com.pennywiseai.tracker.widget.WidgetRefresher.refreshTransactionWidgets(applicationContext)
            }
        } catch (e: Exception) {
            Log.e(TAG, "Historic transfer pairing failed: ${e.message}", e)
        }
    ...
    private suspend fun saveTransaction(
        ...
        coroutineScope {
            ...
            if (hashDeferred.await() != null) {
                // Dedup skip — but recreate the account balance row if the user
                // deleted the account, or a re-scan could never bring it back.
                ensureAccountForDuplicate(parsed, mapped)
            ...
            // Internal-transfer pairing (merchantless debit + equal credit on
            // another same-bank account within minutes = one TRANSFER).
            try {
                transactionRepository.pairInternalTransferIfMatch(finalEntity, rowId)
            } catch (e: Exception) {
     ...
     */
    private suspend fun ensureAccountForDuplicate(parsed: ParsedTransaction, entity: TransactionEntity) {
        val accountLast4 = parsed.accountLast4 ?: return
        val balance = parsed.balance ?: return
        if (parsed.isMobileWallet || parsed.isFromCard) return
        val existing = accountBalanceRepository.getLatestBalance(parsed.bankName, accountLast4)
        ...
        Log.d(TAG, "Recreated deleted account ${parsed.bankName} **$accountLast4 from duplicate SMS")
    }
    ...
    private suspend fun saveRuleApplications(
        ...
        val (add, remove) = ruleApps.tagChanges()
        if (add.isNotEmpty() || remove.isNotEmpty()) tagRepository.applyTagChanges(transactionId, add, remove)
    }
