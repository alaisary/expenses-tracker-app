...
class SmsTransactionProcessor @Inject constructor(
    ...
    private val pendingCategoryReviewBus: PendingCategoryReviewBus,
    ...
    data class ProcessingResult(
        ...
        /** The category the row was actually saved with (post-mapping/rules). */
    ...
    suspend fun processAndSaveTransaction(
        ...
        try {
            ...
            // Saved into the fallback "Others" category → prompt for a pick.
            // Foreground: the sheet pops over the current screen; background:
            // the event buffers until the next app open (the transaction
            // notification already carries quick-category actions meanwhile).
            if (result.success &&
                result.finalCategory == PendingCategoryReviewBus.UNCATEGORIZED
            ) {
                result.transactionId?.let { pendingCategoryReviewBus.enqueue(it) }
            }
    ...
    suspend fun saveParsedTransaction(
        ...
        return try {
            ...
            if (existingTransaction != null) {
                ...
                // Still recreate the account balance row if the user deleted the
                // account — a re-scan hits this path for every stored SMS and must
                // resurrect the account, not skip it.
                Log.d(TAG, "Transaction already exists: ${entity.transactionHash}")
                ensureAccountForDuplicate(parsedTransaction, entity)
            ...
            if (rowId != -1L) {
                ...
                // Internal-transfer pairing: a merchantless debit + equal credit
                // on another same-bank account within minutes is one transfer,
                // not an expense + income. (Runs after insert; sees the
                // counterpart whether it arrived before or after this SMS.)
                try {
                    if (transactionRepository.pairInternalTransferIfMatch(finalEntity, rowId)) {
                        Log.d(TAG, "Paired internal transfer for ${finalEntity.bankName} **${finalEntity.accountNumber}")
                    }
                } catch (e: Exception) {
                    Log.w(TAG, "Transfer pairing failed: ${e.message}")
                }
                ...
                return ProcessingResult(true, transactionId = rowId, finalCategory = finalEntity.category)
            } else {
    ...
    /**
     * Re-creates a deleted account row from a duplicate SMS. Dedup skips the
     * transaction (and its balance update), so without this a deleted account
     * could never come back — not even by re-scanning the SMS history.
     * Create-only-if-missing: never rewrites an existing balance.
     */
    private suspend fun ensureAccountForDuplicate(
        parsedTransaction: ParsedTransaction,
        entity: TransactionEntity
    ) {
        val accountLast4 = parsedTransaction.accountLast4 ?: return
        val balance = parsedTransaction.balance ?: return
        if (parsedTransaction.isMobileWallet || parsedTransaction.isFromCard) return
        val existing = accountBalanceRepository.getLatestBalance(
            parsedTransaction.bankName, accountLast4
        )
        ...
        accountBalanceRepository.insertBalance(
            AccountBalanceEntity(
                ...
                creditLimit = null,
                isCreditCard = false,
                smsSource = entity.smsBody?.take(500),
                sourceType = "TRANSACTION",
                currency = parsedTransaction.currency,
                profileId = ProfileEntity.PERSONAL_ID
            )
        )
        Log.d(TAG, "Recreated deleted account ${parsedTransaction.bankName} **$accountLast4 from duplicate SMS")
    }
