...
interface TransactionDao {
    ...
    // A merchantless debit on one account and an equally-sized credit on
    // another of the same bank, minutes apart, is the bank's own SMS for the
    // two halves of an internal transfer. Counting both as expense+income
    // inflates spending; they should be one TRANSFER. (Alizz/BM "…debited
    // from your account MASK…" templates carry no destination info — the pair
    // is the only signal.)

    /**
     * The best counterpart for an internal transfer: opposite type, same bank
     * + amount + currency, different account, merchantless (real merchant
     * spends and payer-named credits never pair), un-paired so far, within
     * the given time window. Amounts compare numerically — the TEXT storage
     * writes "100" and "100.000" for the same number depending on which
     * template the bank used.
     */
    @Query(
        """SELECT * FROM transactions
        WHERE is_deleted = 0
          AND bank_name = :bankName
          AND CAST(amount AS REAL) = CAST(:amount AS REAL)
          AND currency = :currency
          AND transaction_type = :oppositeType
          AND account_number IS NOT NULL
          AND account_number != :accountNumber
          AND (merchant_name IS NULL OR merchant_name = '' OR merchant_name LIKE 'your account%')
          AND from_account IS NULL AND to_account IS NULL
          AND date_time BETWEEN :from AND :to
        ORDER BY date_time
        LIMIT 1"""
    )
    suspend fun findInternalTransferCounterpart(
        bankName: String,
        amount: String,
        currency: String,
    ...
    /** Both sides of already-paired transfers and future candidates. */
    @Query(
        """SELECT * FROM transactions
        WHERE is_deleted = 0
    ...
    suspend fun getInternalTransferCandidates(): List<TransactionEntity>

    @Query(
        """UPDATE transactions
        SET transaction_type = 'TRANSFER', from_account = :fromAccount,
            to_account = :toAccount, updated_at = :updatedAt
        WHERE id IN (:ids)"""
    )
    suspend fun markInternalTransfers(
        ids: List<Long>,
        fromAccount: String,
        toAccount: String,
    ...
    /**
     * All live transactions in [category] on/after [since], newest first. Used
     * by the category detail screen, which aggregates in Kotlin so it can sum a
     * single currency at a time (summing across currencies in SQL would violate
     * the currency rule).
     */
    @Query("""
        SELECT * FROM transactions
        ...
        ORDER BY date_time DESC
    """)
    suspend fun getCategoryTransactionsSince(category: String, since: LocalDateTime): List<TransactionEntity>
}
