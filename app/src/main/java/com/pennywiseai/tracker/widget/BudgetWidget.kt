...
class BudgetWidget : GlanceAppWidget() {
    ...
    private fun NoBudgetContent() {
        ...
        Column(
            ...
            Text(
                text = context.getString(R.string.wid_monthly_budget),
                style = TextStyle(
            ...
            Text(
                text = context.getString(R.string.wid_tap_to_set_up),
                style = TextStyle(
    ...
    private fun BudgetOverviewContent(data: BudgetWidgetData) {
        ...
        Row(
            ...
            Text(
                text = context.getString(R.string.wid_percent_used, data.percentageUsed.toInt()),
                style = TextStyle(
                    color = statusColor,
                    fontSize = 13.sp,
        ...
        if (!isSmall) {
            ...
            Row(
                ...
                Text(
                    text = context.getString(
                        R.string.wid_of_amount,
                        CurrencyFormatter.formatCurrency(data.totalLimit, data.currency)
                    ),
                    style = TextStyle(
                        color = GlanceTheme.colors.onSurfaceVariant,
                        fontSize = 12.sp
                ...
                val remainingText = if (data.remaining >= BigDecimal.ZERO) {
                    context.getString(
                        R.string.wid_amount_left,
                        CurrencyFormatter.formatCurrency(data.remaining, data.currency)
                    )
                } else {
                    context.getString(
                        R.string.wid_amount_over,
                        CurrencyFormatter.formatCurrency(data.remaining.abs(), data.currency)
                    )
                }
                Text(
                    text = remainingText,
            ...
            Row(
                ...
                if (data.dailyAllowance > BigDecimal.ZERO) {
                    Text(
                        text = context.getString(
                            R.string.wid_per_day,
                            CurrencyFormatter.formatCurrency(data.dailyAllowance, data.currency)
                        ),
                        style = TextStyle(
                            color = GlanceTheme.colors.onSurfaceVariant,
                            fontSize = 12.sp
                ...
                if (data.totalIncome > BigDecimal.ZERO) {
                    ...
                    val savingsText = buildString {
                        append(
                            context.getString(
                                if (data.netSavings >= BigDecimal.ZERO) R.string.wid_saved else R.string.wid_over,
