...
enum class CategoryViewMode { CATEGORY, GROUP }
...
class AnalyticsViewModel @Inject constructor(
    ...
    private val categoryIndex: Flow<CategoryIndex> =
        categoryRepository.getAllCategories()
            .map { cats ->
                CategoryIndex(
                    colors = cats.associate { it.name to it.color },
                    icons = cats.associate { it.name to it.icon },
                    parentOf = cats.parentNameOf(),
                    all = cats
                )
            }
    ...
    private val _selectedCategoryViewMode = MutableStateFlow(CategoryViewMode.CATEGORY)
    val selectedCategoryViewMode: StateFlow<CategoryViewMode> = _selectedCategoryViewMode.asStateFlow()
    ...
    val uiState: StateFlow<AnalyticsUiState> = combine(
        ...
        val categoryIconMap = catIndex.icons
        ...
        // Only the selected sub-category is exempt; other children (e.g. the
        // Groceries half of a Coffee/Groceries split) still roll into their parent.
        val parentOf = catIndex.parentOf - listOfNotNull(filterState.categoryFilter)
        ...
        if (dateRange == null) {
            ...
            transactionRepository.getTransactionsBetweenDates(
                ...
                val categoryBreakdownByGroup = buildGroupBreakdown(categoryBreakdown, totalSpending)
                ...
                AnalyticsUiState(
                    ...
                    categoryBreakdownByGroup = categoryBreakdownByGroup,
    ...
    fun setCategoryViewMode(mode: CategoryViewMode) {
        _selectedCategoryViewMode.value = mode
    }
...
data class AnalyticsUiState(
    ...
    // Category spending rolled up into [CategoryGroup] buckets (EXCLUDED omitted,
    // since transfers/credit-card payments are not spending). Same denominator
    // convention as [categoryBreakdown].
    val categoryBreakdownByGroup: List<CategoryData> = emptyList(),
...
data class CategoryData(
    ...
    // Optional user-chosen category emoji (#760), used to render the category
    // tile. Null means the category has no custom emoji.
...
/**
 * Rolls a category breakdown up into [CategoryGroup] buckets. [CategoryGroup.EXCLUDED]
 * (transfers, credit-card payments) is dropped because moving money between your own
 * accounts is not spending. Percentages reuse the existing [totalSpending] denominator
 * and are clamped at 100% exactly like [CategoryData.percentage].
 */
private fun buildGroupBreakdown(
    categoryBreakdown: List<CategoryData>,
    totalSpending: BigDecimal
): List<CategoryData> {
    val amounts = mutableMapOf<CategoryGroup, BigDecimal>()
    val counts = mutableMapOf<CategoryGroup, Int>()

    for (category in categoryBreakdown) {
        val group = groupOf(category.name)
        if (group == CategoryGroup.EXCLUDED) continue
        amounts[group] = (amounts[group] ?: BigDecimal.ZERO) + category.amount
        counts[group] = (counts[group] ?: 0) + category.transactionCount
    }

    return amounts.map { (group, amount) ->
        CategoryData(
            name = group.name,
            amount = amount,
            percentage = if (totalSpending > BigDecimal.ZERO) {
                (amount.divide(totalSpending, 4, java.math.RoundingMode.HALF_UP) * BigDecimal(100)).toFloat().coerceAtMost(100f)
            } else 0f,
