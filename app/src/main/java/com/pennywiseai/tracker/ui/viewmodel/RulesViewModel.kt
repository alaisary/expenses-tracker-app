...
class RulesViewModel @Inject constructor(
    ...
    fun reportNothingToExport() {
        _sharingMessage.value = context.getString(R.string.vm_rules_nothing_to_export)
    }
    ...
    private fun readBoundedText(uri: Uri): String {
            ...
            ?.use { cursor ->
                ...
                if (cursor.moveToFirst() && sizeIndex >= 0 && !cursor.isNull(sizeIndex)) {
                    require(cursor.getLong(sizeIndex) <= limit) {
                        context.getString(R.string.vm_rules_file_too_large)
                    }
                }
            }

        val input = context.contentResolver.openInputStream(uri)
            ?: throw IllegalStateException(context.getString(R.string.vm_rules_open_failed))
        return input.use { stream ->
            ...
            require(read <= limit) { context.getString(R.string.vm_rules_file_too_large) }
        ...
        viewModelScope.launch {
            try {
                ...
                withContext(Dispatchers.IO) {
                    ...
                    } ?: throw IllegalStateException(context.getString(R.string.vm_rules_open_write_failed))
                }
                _sharingMessage.value = if (exportable.size == 1) {
                    context.getString(R.string.vm_rules_exported_one)
                } else {
                    context.getString(R.string.vm_rules_exported_count, exportable.size)
                }
            } catch (e: Exception) {
                _sharingMessage.value = context.getString(R.string.vm_rules_export_failed, e.message)
            }
        }
    }
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                _sharingMessage.value = buildString {
                    append(
                        if (toImport.size == 1) context.getString(R.string.vm_rules_imported_one)
                        else context.getString(R.string.vm_rules_imported_count, toImport.size)
                    )
                    if (duplicates > 0) append(" " + context.getString(R.string.vm_rules_duplicates_existed, duplicates))
                    if (skipped > 0) {
                        append(
                            " " + if (skipped == 1) context.getString(R.string.vm_rules_one_repeated_collapsed)
                            else context.getString(R.string.vm_rules_repeated_collapsed, skipped)
                        )
                    }
                    if (blocked > 0) {
                        append(" " + context.getString(R.string.vm_rules_blocked_pro, blocked, FreeTierLimits.MAX_RULES))
                    }
                }
            } catch (e: Exception) {
                _sharingMessage.value = e.message ?: context.getString(R.string.vm_rules_import_failed)
            } finally {
                _isLoading.value = false
            }
        }
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                _batchApplyResult.value = BatchApplyResult(
                    totalProcessed = 0,
                    totalUpdated = 0,
                    errors = listOf(context.getString(R.string.vm_error_prefix, e.message))
                )
            } finally {
                _isLoading.value = false
