                ...
                selectedTags.forEach { tag ->
                    InputChip(
                        ...
                        trailingIcon = {
                            Icon(
                                Icons.Default.Close,
                                contentDescription = stringResource(R.string.comp_remove_tag, tag),
            ...
            trailingIcon = {
                if (trimmed.isNotEmpty()) {
                    IconButton(onClick = { commit(input) }) {
                        Icon(
                            Icons.Default.Add,
                            contentDescription = stringResource(R.string.comp_add_tag),
                            modifier = Modifier.size(Dimensions.Icon.medium)
                        )
                    }
                }
            ...
            FlowRow(horizontalArrangement = Arrangement.spacedBy(Spacing.sm)) {
                ...
                if (showCreateOption) {
                    AssistChip(
                        ...
                        label = {
                            Text(
                                stringResource(R.string.comp_create_tag, trimmed),
                                maxLines = 1,
                                overflow = TextOverflow.Ellipsis
                            )
                        }
