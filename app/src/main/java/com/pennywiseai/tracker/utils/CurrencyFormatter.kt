...
object CurrencyFormatter {
    ...
    fun formatCurrency(amount: BigDecimal, currencyCode: String = "INR"): String {
        ...
        // Amount first, currency mark after it; under RTL that puts the mark on
        // the left of the figure.
        if (currencyCode == "OMR") {
            val number = formatAmount(amount, currencyCode)
    ...
    /**
     * True when the app is currently displaying Arabic, used to place the
     * currency mark after the amount (on the left in RTL).
     */
    private fun isArabicLocale(): Boolean =
        java.util.Locale.getDefault().language == "ar"

    /**
     * Formats only the numeric amount (locale grouping + correct decimals) with
     * no currency symbol. Used where the currency mark is rendered separately —
     * e.g. the official Omani Rial sign drawn as an inline vector.
     */
    fun formatAmountOnly(amount: BigDecimal, currencyCode: String): String =
    ...
    fun formatAbbreviated(value: Double, currencyCode: String): String {
        ...
        // can't be inlined here — use the ISO code rather than the Arabic mark.
        val symbol = if (currencyCode == "OMR") "OMR" else getCurrencySymbol(currencyCode)
