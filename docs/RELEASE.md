...
3. Builds, renames, and computes SHA256 for the APKs.
4. Commits `chore(release): bump version to X [skip ci]`, tags `vX`, and pushes.
5. Cuts the GitHub release with the universal / arm64-v8a / armeabi-v7a APKs +
   `.sha256` attached (x86 and x86_64 builds are produced and hashed but kept
   local).
